"""Map pct_software exposure scores to workers and summarize by status."""
...
SOFTWARE_EXPOSURE_PATH = Path("data/exposure_by_occ1990dd_lswt2010.xls")
SOFTWARE_EXPOSURE_SCORE_COL = "pct_software"
SOFTWARE_OUTPUT_PATH = Path(
    "outputs/tables/software_exposure_pct_software_by_worker_status_summary.csv"
)
SOFTWARE_FIGURE_DIR = Path("outputs/figures")
...
def attach_software_exposure(df, occ_col, lookup):
    out = df.copy()
    out["software_occ1990dd_code"] = pd.to_numeric(
        out[occ_col],
        errors="coerce",
    ).astype("Int64")
    out["pct_software"] = out["software_occ1990dd_code"].map(
        lookup["pct_software"]
    )
    out["pct_software_occupation_title"] = out["software_occ1990dd_code"].map(
...
if "ipums_df" not in globals() or "foreign_born_df" not in globals():
    raise RuntimeError(
        "Run the ACS data and likely-legal classification cells before this section."
    )
...
software_exposure_raw = pd.read_csv(SOFTWARE_EXPOSURE_PATH)
software_exposure_scores = (
    software_exposure_raw
    .rename(columns={
        "occ1990dd": "occ1990dd",
        "occ1990dd_title": "occ1990dd_title",
        SOFTWARE_EXPOSURE_SCORE_COL: "pct_software",
    })
    .dropna(subset=["occ1990dd", "pct_software"])
    .copy()
)
software_exposure_scores["occ1990dd"] = pd.to_numeric(
    software_exposure_scores["occ1990dd"],
    errors="coerce",
).astype("Int64")
software_exposure_scores["pct_software"] = pd.to_numeric(
    software_exposure_scores["pct_software"],
    errors="coerce",
)
software_exposure_scores["occ1990dd_title"] = (
    software_exposure_scores["occ1990dd_title"].astype(str).str.strip()
)
software_exposure_scores = software_exposure_scores.dropna(
    subset=["occ1990dd", "pct_software"]
)

software_exposure_lookup = (
    software_exposure_scores
...
ipums_software_occ_col = find_occ1990dd_column(ipums_df)
foreign_software_occ_col = find_occ1990dd_column(foreign_born_df)

if ipums_software_occ_col is None or foreign_software_occ_col is None:
    available_occ_cols = sorted([col for col in ipums_df.columns if "OCC" in col.upper()])
    print(
        f"Loaded {len(software_exposure_scores):,} pct_software exposure scores "
        f"from {SOFTWARE_EXPOSURE_PATH}."
    )
    print("These scores are keyed by occ1990dd.")
    print(f"Current ACS extract occupation columns: {available_occ_cols}")
    raise KeyError(
        "The current ACS extract does not include OCC1990DD/OCC1990, so "
        "pct_software cannot be mapped yet. Add OCC1990/OCC1990DD to the IPUMS "
        "extract or provide a crosswalk from OCC/OCC2010 to occ1990dd."
    )

if (
    ipums_software_occ_col.upper() == "OCC1990"
    or foreign_software_occ_col.upper() == "OCC1990"
):
    print(
        "Using OCC1990 as the occ1990dd key for pct_software. Please verify this "
        "matches the exposure file's occupation coding."
    )

ipums_df = attach_software_exposure(
    ipums_df,
    ipums_software_occ_col,
    software_exposure_lookup,
)
foreign_born_df = attach_software_exposure(
    foreign_born_df,
    foreign_software_occ_col,
    software_exposure_lookup,
)
...
for label, source_df in worker_status_groups.items():
    ...
    workers_with_scores = workers[workers["pct_software"].notna()].copy()
    score_weights = (
        workers_with_scores
        .groupby("pct_software", dropna=True)[WEIGHT_COL]
        .sum()
        .sort_index()
    )
    ...
    if len(score_weights) > 0 and weighted_workers_with_scores > 0:
        weighted_mean_pct_software = np.average(
            score_weights.index.to_numpy(dtype=float),
            weights=score_weights.to_numpy(dtype=float),
        )
    else:
        weighted_mean_pct_software = np.nan
    ...
    summary_rows.append({
        ...
        "weighted_mean_pct_software": weighted_mean_pct_software,
    })

software_exposure_status_summary = pd.DataFrame(summary_rows)

SOFTWARE_OUTPUT_PATH.parent.mkdir(parents=True, exist_ok=True)
software_exposure_status_summary.to_csv(SOFTWARE_OUTPUT_PATH, index=False)
...
if not score_value_arrays:
    raise ValueError("No employed workers have mapped pct_software exposure scores.")
...
ax.set_title(
    "Software Exposure Score Distribution by Worker Status",
    fontsize=15,
    fontweight="bold",
)
ax.set_xlabel("Software exposure score", fontsize=12)
...
SOFTWARE_FIGURE_DIR.mkdir(parents=True, exist_ok=True)
software_exposure_kde_png = (
    SOFTWARE_FIGURE_DIR / "software_exposure_kde_by_worker_status.png"
)
software_exposure_kde_svg = (
    SOFTWARE_FIGURE_DIR / "software_exposure_kde_by_worker_status.svg"
)
fig.savefig(software_exposure_kde_png, dpi=300, bbox_inches="tight")
fig.savefig(software_exposure_kde_svg, bbox_inches="tight")
plt.show()
...
print(f"Saved software exposure status summary to {SOFTWARE_OUTPUT_PATH}.")
print(f"Saved figure to {software_exposure_kde_png} and {software_exposure_kde_svg}.")
show_table(software_exposure_status_summary)
